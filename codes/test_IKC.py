...
for test_loader in test_loaders:
    ...
    for test_data in test_loader:
            ...
            if need_GT:
                ...
                if gt_img.shape[2] == 3:  # RGB image
                    ...
                    if not math.isnan(ssim):
                        single_img_psnr.append(float(psnr)) # single_img_psnr += psnr
                        single_img_ssim.append(float(ssim)) # single_img_ssim += ssim
                        single_img_psnr_y.append(float(psnr_y)) # single_img_psnr_y += psnr_y
                        single_img_ssim_y.append(float(ssim_y)) # single_img_ssim_y += ssim_y
                    else:
                        single_img_nan_num += 1
                    logger.info(
        ...
        # single_img_psnr = np.array(single_img_psnr)
        # single_img_ssim = np.array(single_img_psnr)
        # single_img_psnr_y = np.array(single_img_psnr_y)
        # single_img_ssim_y = np.array(single_img_ssim_y)

        if len(single_img_psnr) != 0 and need_GT:
            max_img_index = np.argmax(single_img_psnr)
            test_results['nan_num'] += int(single_img_nan_num / opt_C['step'])
    ...
    if need_GT:  # metrics
        ...
        logger.info(
            '----Average PSNR/SSIM results for {}----\n\tPSNR: {:.6f} dB; SSIM: {:.6f}, num of the case that value is nan: {}\n'.format(
                test_set_name, ave_psnr, ave_ssim, test_results['nan_num']))
